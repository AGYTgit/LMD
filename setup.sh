...
for moduleName in "${modulesToInstall[@]}"; do
    ...
    (
        ...
        if [[ -n "$PACMAN_DEPS" ]]; then
            ...
            echo "  ${YELLOW}Warning:${RESET} No pacman dependencies detected."
        fi

        if [[ -n "$YAY_DEPS" ]]; then
            echo "  ${CYAN}yay:${RESET} ${YAY_DEPS[*]}"
            yay -S "${YAY_DEPS[@]}" --needed
        else
            echo "  ${YELLOW}Warning:${RESET} No yay dependencies detected."
        fi
        echo
        ...
        if [[ -n "$SYM_SOURCE_DF" && -n "$SYM_TARGET_DF" ]]; then
            ...
            echo "  ${GREEN}Symlinking:${RESET} ${CYAN}$SYM_SOURCE_DF${RESET} -> ${CYAN}$SYM_TARGET_DF${RESET}"
            ln -sr "$SYM_SOURCE_DF" "$SYM_TARGET_DF"
        else
            echo "  ${YELLOW}Warning:${RESET} No source/target for symlink detected."
        fi
        echo
...
if [[ ${#failed[@]} -gt 0 ]]; then
    echo "${RED}Failed modules:${RESET} ${MAGENTA}${failed[*]}${RESET}"
else
    echo "${GREEN}No failed modules.${RESET}"
fi

if [[ ${#completed[@]} -gt 0 ]]; then
    echo "${GREEN}Successfully installed modules:${RESET} ${MAGENTA}${completed[*]}${RESET}"
else
